...
void ConsoleUI::RenderMonitor(AppConfig& config, CpuMonitor& cpuMon) {
    ...
    SetColor(BLACK);
    std::wstring f1 = (config.lang == Language::Ukrainian ? L"F1Довідка " : L"F1Help    ");
    std::wstring f2 = (config.lang == Language::Ukrainian ? L"F2Мова    " : L"F2Lang    ");
    std::wstring f6 = (config.lang == Language::Ukrainian ? L"F6Інтервал" : L"F6Interval");
    std::wstring f9 = (config.lang == Language::Ukrainian ? L"F9Заверш  " : L"F9Kill    ");
