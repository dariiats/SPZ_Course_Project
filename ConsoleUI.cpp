...
void ConsoleUI::RenderMonitor(AppConfig& config, CpuMonitor& cpuMon) {
    ...
    if (config.showSearch || config.showFilter) {
        ...
        std::wcout << VT_BG_DARKGRAY << VT_FG_BRIGHT_WHITE << L" F5 " << VT_BG_CYAN << VT_FG_BLACK << (config.treeView ? (config.lang == Language::Ukrainian ? L"Дерево" : L"Tree *") : (config.lang == Language::Ukrainian ? L"Дерево" : L"Tree  "));
