...
void ConsoleUI::RenderMonitor(AppConfig& config, CpuMonitor& cpuMon) {
    ...
    if (config.activeTab == TabView::Main) {
        ...
        switch (config.sortColumn) {
            case SortColumn::Pid:    mPid = L" PID\x25BC"; break;
            case SortColumn::Cpu:    mCpu = L"CPU\x25BC"; break;
            case SortColumn::Memory: mMem = L"MEM\x25BC"; break;
            case SortColumn::Time:   mTime = L"TIME\x25BC"; break;
            case SortColumn::Name:   mCmd = (config.lang == Language::Ukrainian ? L"\x25BCКОМАНДА" : L"\x25BCCOMMAND"); break;
        }
